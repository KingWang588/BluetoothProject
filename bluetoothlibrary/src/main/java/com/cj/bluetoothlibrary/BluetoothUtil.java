...
public class BluetoothUtil {
    ...
    private static BluetoothDevice CONNECT_MAC;
     ...
     * 蓝牙信号强度
     */
    public void onReadRssi(String deviceAddress) {
            ...
            mBluetoothClient.readRssi(deviceAddress, new BleReadRssiResponse() {
                ...
                public void onResponse(int code, Integer rssi) {
                    if (code == REQUEST_SUCCESS) {
                        scanBluetoothListener.onReadBluetoothRssi(rssi);
                    }
                }
            });
        }
     ...
     * 扫描蓝牙，如果有连接，进行断开
     */
    public void isConnectDetection(String  macAddress) {
        BluetoothDevices devices = SpHelper.getBluetooth(context);
        if (devices != null) {
            //判断是否是同一蓝牙
            if (!macAddress.equals(devices.getAddress())) {
                //不是的话判断蓝牙连接状态
    ...
    public void unregisterBluetoothStateListener() {
        ...
        if (bluetoothDevices != null) {
            ...
            //是否有连接的蓝牙，有的话先断开
            if (CONNECT_MAC != null) {
                mBluetoothClient.disconnect(CONNECT_MAC.getAddress());
            }
        }
        if (CONNECT_MAC != null) {
            //关闭Notify
            ...
            mBluetoothClient.search(request, new SearchResponse() {
                ...
                public void onDeviceFounded(SearchResult device) {
                    ...
                    if (scanBluetoothListener != null) {
                        ...
                        //扫描到设备回调
                        scanBluetoothListener.onDeviceSpyListener(device.device, device.rssi);
     ...
     * 设备连接蓝牙
     */
    public void connectBluetooth(final String deviceName, final String deviceMacAddress, final int type) {
        //停止扫描蓝牙
        stopBluetooth();
        //是否有连接的蓝牙，有的话先断开
        isConnectDetection(deviceMacAddress);
        //再去连接蓝牙
        ...
        mBluetoothClient.connect(deviceMacAddress, options, new BleConnectResponse() {
            @Override
            public void onResponse(int code, BleGattProfile data) {
                ...
                if (code == REQUEST_SUCCESS) {
                   ...
                   // CONNECT_MAC = device;
                    //保存最后一次连接
                    SpHelper.setBluetooth(context, deviceName,deviceMacAddress, type);
                    //设备蓝牙强度
                    onReadRssi(deviceMacAddress);
                    //连接成功监听蓝牙连接状态，防止中途意外断开
                    mBluetoothClient.registerConnectStatusListener(deviceMacAddress, mBleConnectStatusListener);
                    notifyBluetoothData();
                    if (scanBluetoothListener != null) {
                    ...
                    if (scanBluetoothListener != null) {
                        scanBluetoothListener.onDeviceConnectFailing(code);
                    }
                }
            }
        });
        ...
        mBluetoothClient.connect(device.getAddress(), options, new BleConnectResponse() {
            ...
            public void onResponse(int code, BleGattProfile data) {
                ...
                if (code == REQUEST_SUCCESS) {
                    ...
                    //设备蓝牙强度
                    onReadRssi(device.getAddress());
                    //连接成功监听蓝牙连接状态，防止中途意外断开
                    mBluetoothClient.registerConnectStatusListener(CONNECT_MAC.getAddress(), mBleConnectStatusListener);
                    notifyBluetoothData();
                    if (scanBluetoothListener != null) {
                        scanBluetoothListener.onDeviceConnectSucceed();
                    }
    ...
    public void connectAutomaticBluetooth(int type) {
        if (nullBluetooth()) {
            ...
            if (CONNECT_MAC_DATA != null) {
                ...
                stopBluetooth();
                SearchRequest request = new SearchRequest.Builder()
                        .searchBluetoothLeDevice(2000)   // 先扫BLE设备2s
                        .searchBluetoothClassicDevice(2000) // 再扫经典蓝牙2s
                        .searchBluetoothLeDevice(2000)      // 再扫BLE设备2s
                        .build();
                ...
                mBluetoothClient.search(request, new SearchResponse() {
                    ...
                    public void onDeviceFounded(final SearchResult device) {
                        ...
                        //根据传入的名称，分辨设备
                        if (device.device.getName().startsWith(CONNECT_MAC_DATA.getName())) {
                            //停止扫描蓝牙
                            stopBluetooth();
                            //再去连接蓝牙
                            BleConnectOptions options = new BleConnectOptions.Builder()
                                    ...
                                    .setServiceDiscoverTimeout(3000)  // 发现服务超时3s
                                    .build();

                            mBluetoothClient.connect(device.device.getAddress(), options, new BleConnectResponse() {
                                @Override
                                public void onResponse(int code, BleGattProfile data) {
                                    //连接成功
                                    if (code == REQUEST_SUCCESS) {
                                        //连接设备赋值
                                        CONNECT_MAC = device.device;
                                        //设备蓝牙强度
                                        onReadRssi(device.device.getAddress());
                                        //连接成功监听蓝牙连接状态，防止中途意外断开
                                        mBluetoothClient.registerConnectStatusListener(CONNECT_MAC.getAddress(), mBleConnectStatusListener);
                                        notifyBluetoothData();
                                        if (scanBluetoothListener != null) {
    ...
    public void readBluetoothData() {
        mBluetoothClient.read(CONNECT_MAC.getAddress(), UUID_SERVICE, UUID_NOTIFY, new BleReadResponse() {
            @Override
    ...
    public void writeBluetoothData(String data) {
        if (CONNECT_MAC != null) {
            mBluetoothClient.write(CONNECT_MAC.getAddress(), UUID_SERVICE, UUID_WRITE, ByteUtils.stringToBytes(data), new BleWriteResponse() {
                @Override
    ...
    public void writeBluetoothBabyData(String data) {
        if (CONNECT_MAC != null) {
            Log.e("onNotify: ", "Baby发送数据：" + data);
            Log.e("onNotify: ", "Baby发送蓝牙地址" + CONNECT_MAC.getAddress());
            Log.e("onNotify: ", "Baby发送蓝牙地址" + UUID_SERVICE);
            Log.e("onNotify: ", "Baby发送蓝牙地址" + UUID_WRITE);
            mBluetoothClient.writeNoRsp(CONNECT_MAC.getAddress(), UUID_SERVICE, UUID_WRITE, ByteUtils.stringToBytes(data), new BleWriteResponse() {
                @Override
                public void onResponse(int code) {
                    if (code == REQUEST_SUCCESS) {
                    } else {
    ...
    public void notifyBluetoothData() {
        mBluetoothClient.notify(CONNECT_MAC.getAddress(), UUID_SERVICE, UUID_NOTIFY, new BleNotifyResponse() {
    ...
    public void unNotifyBluetoothData() {
        mBluetoothClient.unnotify(CONNECT_MAC.getAddress(), UUID_SERVICE, UUID_NOTIFY, new BleUnnotifyResponse() {
            @Override
            public void onResponse(int code) {
                if (code == REQUEST_SUCCESS) {
